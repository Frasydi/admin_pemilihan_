...
export default function KandidatTimKecamatan() {
    ...
    const {data, isLoading, isError, refetch} = useFetch("/api/tim/"+params.id+type+"?"+new URLSearchParams({search : search}))
                ...
                <Grid container direction={"column"}>
                    ...
                    <Grid item>
                        <Grid container spacing={3}>
                            {
                                typeName == "Kelurahan" && <Grid item xs={3}>
                    ...
                    <Grid item>
                        {
                            (isLoading === true || isError == true ) && <Skeleton width={"100%"} height={"50vh"} />
                        }
                        {
                            (isLoading == false && isError === false)  && <TableTim data={data} header={header[typeName]} keys={keys[typeName]} />
                        }
                    </Grid>
                    </TimContext.Provider>
                </Grid>
